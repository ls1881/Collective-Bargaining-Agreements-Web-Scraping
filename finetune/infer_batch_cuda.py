...
def stage_translate():
    ...
    approach) and a much larger MAX_CHARS (8000, up from the notebook's 2000). Measured
    empirically: request latency to the unofficial googletrans endpoint is roughly constant
    (~1-1.5s) regardless of payload size up to ~14,200-14,400 chars, where it silently starts
    FAILING — returning the original untranslated text with no exception raised (confirmed
    byte-for-byte identical output). That threshold isn't a fixed constant either: it varied by
    document in testing, so 8000 was chosen with real margin below the observed failure zone on
    multiple real corpus documents, cutting total chunk count ~4x versus the 2000-char baseline.
